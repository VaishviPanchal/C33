...
function setup(){
    ...
    sling = new SlingShot(birds[birds.length-1].body,{x:200,y:50})
...
function mouseReleased(){
    gameState="launch"
    sling.fly();
    Matter.Body.applyForce(birds[birds.length-1].body,birds[birds.length-1].body.position,{x:10,y:-20})
    birds.pop()
}
function keyPressed(){
    if (keyCode===32){
        gameState="onSling"
        if(birds[birds.length-1]!==undefined){
        //if(birds.length>0){   
